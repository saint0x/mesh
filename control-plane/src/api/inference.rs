...
use crate::api::types::{
    ...
    ClaimInferenceAssignmentResponse, DecodeLeaseSessionMember, DecodeLeaseStatus,
    DownloadInferenceSessionCheckpointResponse, DownloadInferenceSessionKvTransferPayloadResponse,
    InferenceExecutionLease, InferenceExecutionPlan, InferenceJobAssignmentStatus,
    InferenceJobStatusResponse, InferenceSchedulerQueueState, InferenceSessionCheckpointPayload,
    ...
    ReportInferenceAssignmentRequest, ReportInferenceSessionKvTransferRequest,
    ReportInferenceSessionKvTransferResponse, ServingSessionMetadata, SubmitInferenceRequest,
    SubmitInferenceResponse, SupportGroupRole, UploadInferenceSessionCheckpointRequest,
...
use crate::services::{
    device_metadata_from_capabilities, refresh_decode_plan_for_job, select_claim_assignment_id,
...
#[derive(Debug, Clone, serde::Deserialize)]
pub struct ObservePendingKvTransfersQuery {
    device_id: String,
    network_id: String,
}

#[derive(Debug, Clone, serde::Deserialize)]
pub struct DownloadKvTransferPayloadQuery {
    device_id: String,
}
...
async fn submit_inference_once(
    ...
    let scheduling_policy = state.get_network_scheduling_policy(&req.network_id)?;
...
pub async fn claim_inference_assignment(
    ...
    let execution_lease = if let Some(assignment) = assignment.clone() {
        ...
        let lease_session_members =
            load_decode_lease_session_members(&conn, &req.device_id, &lease_session_ids)?;
        Some(build_execution_lease(
            assignment,
            lease_session_ids,
...
pub async fn observe_pending_kv_transfers(
    State(state): State<AppState>,
    Query(query): Query<ObservePendingKvTransfersQuery>,
) -> ApiResult<Json<ObservePendingKvTransfersResponse>> {
    if query.device_id.is_empty() || query.network_id.is_empty() {
        return Err(ApiError::BadRequest(
            "device_id and network_id must be provided".to_string(),
        ));
    ...
    Ok(Json(ObservePendingKvTransfersResponse {
        success: true,
        transfers,
    }))
}
...
fn build_execution_lease(
    ...
    lease_session_ids: Vec<String>,
    lease_session_members: Vec<DecodeLeaseSessionMember>,
    ...
    let canonical_lease_session_ids = if lease_session_members.is_empty() {
        lease_session_ids
    } else {
        lease_session_members
            .iter()
    ...
    Ok(InferenceExecutionLease {
        ...
        session: InferenceSessionLease {
            ...
            lease_session_ids: canonical_lease_session_ids,
            lease_session_members,
...
fn load_serving_session_metadata(
    ...
    session.kv_checkpoint_device_id = session
        .checkpoint
        .as_ref()
        .map(|checkpoint| checkpoint.source_device_id.clone());
    session.kv_checkpoint_created_at = session
        .checkpoint
        .as_ref()
    ...
    let transfer_rows = load_pending_transfer_rows_for_session(conn, &session.session_id)?;
    let queue_row = conn
        .query_row(
...
fn load_pending_transfer_rows_for_session(
    conn: &rusqlite::Connection,
    session_id: &str,
) -> ApiResult<Vec<crate::api::types::KvTransferStatus>> {
    let mut stmt = conn
        .prepare(
            r#"
    ...
    let rows = stmt
        .query_map(params![session_id], |row| {
            Ok(crate::api::types::KvTransferStatus {
...
fn load_pending_kv_transfers(
    conn: &rusqlite::Connection,
    network_id: &str,
    device_id: &str,
) -> ApiResult<Vec<PendingKvTransferStatus>> {
    let mut stmt = conn
        .prepare(
            r#"
            SELECT t.transfer_id, t.session_id, t.job_id, t.network_id, t.model_id, t.segment_id,
                   t.group_id, t.batch_group_key, t.source_device_id, t.target_device_id,
                   t.transfer_kind, t.status, t.remote_access_uri, t.kv_sequence_position, t.bytes_total,
                   t.bytes_transferred, t.started_at, t.completed_at, t.updated_at, t.last_error,
                   EXISTS(
                       SELECT 1
                       FROM inference_session_kv_transfer_payloads p
                       WHERE p.transfer_id = t.transfer_id
                   ) AS payload_available
            FROM inference_session_kv_transfers t
            WHERE t.network_id = ?
              AND t.status IN ('queued', 'in_progress')
              AND (t.source_device_id = ? OR t.target_device_id = ?)
            ORDER BY t.updated_at DESC, t.transfer_id DESC
            "#,
        )
        .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
    let rows = stmt
        .query_map(params![network_id, device_id, device_id], |row| {
            Ok(PendingKvTransferStatus {
                transfer_id: row.get(0)?,
                session_id: row.get(1)?,
                job_id: row.get(2)?,
                ...
                transfer_kind: row.get(10)?,
                status: row.get(11)?,
                remote_access_uri: row.get(12)?,
                kv_sequence_position: row.get::<_, Option<i64>>(13)?.map(|v| v as u32),
                bytes_total: row.get::<_, Option<i64>>(14)?.map(|v| v as u64),
                bytes_transferred: row.get::<_, Option<i64>>(15)?.map(|v| v as u64),
                started_at: row.get(16)?,
                completed_at: row.get(17)?,
                updated_at: row.get(18)?,
                last_error: row.get(19)?,
                payload_available: row.get::<_, i64>(20)? != 0,
            })
        })
        .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
    rows.collect::<Result<Vec<_>, _>>()
...
fn serialize_kv_residency_kind(kind: crate::api::types::KvResidencyKind) -> ApiResult<String> {
    serde_json::to_string(&kind)
        .map(|value| value.trim_matches('"').to_string())
        .map_err(|e| ApiError::Internal(format!("Failed to serialize kv residency kind: {}", e)))
}

fn build_remote_checkpoint_uri(job_id: &str, session_id: &str) -> String {
    format!(
...
fn prompt_cache_key(prompt_tokens: &[u32]) -> String {
    use sha2::Digest;
...
fn prompt_overlap_tokens(existing_tokens_json: &str, prompt_tokens: &[u32]) -> u32 {
    let Ok(existing_tokens) = serde_json::from_str::<Vec<u32>>(existing_tokens_json) else {
        return 0;
    };
    existing_tokens
        .iter()
...
fn initialize_session_kv_tracking(
    ...
    let prompt_tokens_json = serde_json::to_string(prompt_tokens)
        .map_err(|e| ApiError::Internal(format!("Failed to serialize prompt tokens: {}", e)))?;
    let cache_key = prompt_cache_key(prompt_tokens);
    let overlap_token_count = {
        let mut stmt = tx
            .prepare(
                r#"
                SELECT prompt_tokens_json
                FROM inference_prompt_cache_entries
                WHERE network_id = ? AND model_id = ? AND status = 'ready'
                "#,
            )
            .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        let rows = stmt
            .query_map(params![network_id, model_id], |row| row.get::<_, String>(0))
            .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        let mut max_overlap = 0_u32;
        for row in rows {
            let existing =
                row.map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
            max_overlap = max_overlap.max(prompt_overlap_tokens(&existing, prompt_tokens));
        }
        max_overlap
    };
    ...
    tx.execute(
        ...
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, 'planned', NULL, ?, ?, ?, NULL)
        "#,
        params![
            &initial_segment.session_id,
            job_id,
    ...
    for segment in &execution_plan.segments {
        let group = execution_plan
            ...
            .ok_or_else(|| {
                ApiError::Internal(format!(
                    "Execution group {} missing from execution plan {}",
                    segment.execution_group_id, execution_plan.plan_id
                ))
            })?;
        let participants = segment
            .participant_device_ids
            .iter()
            .cloned()
            .collect::<HashSet<_>>();
        for member in &group.members {
            let residency_kind = if member.device_id == segment.kv_owner_device_id {
                crate::api::types::KvResidencyKind::CheckpointStore
            } else {
                crate::api::types::KvResidencyKind::RemoteReference
            };
            tx.execute(
                r#"
                ...
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'planned', NULL, NULL, NULL, NULL, NULL, NULL, ?, ?, 0, ?, ?, ?, ?, NULL)
                "#,
                params![
                    &segment.session_id,
                    job_id,
                    network_id,
                    model_id,
                    serialize_execution_phase(segment.phase),
                    &group.group_id,
                    &member.device_id,
                    member.shard.column_start as i64,
                    member.shard.column_end as i64,
                    &segment.kv_owner_device_id,
                    serialize_kv_residency_kind(residency_kind)?,
                    &cache_key,
                    prompt_tokens.len() as i64,
                    if matches!(segment.phase, crate::api::types::ExecutionPhase::Decode)
                        && participants.contains(&member.device_id)
                    {
                        1_i64
                    } else {
                        0_i64
...
fn plan_session_kv_transfers_for_decode_segment(
    tx: &rusqlite::Transaction<'_>,
    job_id: &str,
    network_id: &str,
    model_id: &str,
    ...
    target_device_ids: &HashSet<String>,
    now: &str,
) -> ApiResult<()> {
...
fn materialize_session_kv_tracking(
    tx: &rusqlite::Transaction<'_>,
    job_id: &str,
    session_id: &str,
    source_device_id: &str,
    ...
    let remote_access_uri = build_remote_checkpoint_uri(job_id, session_id);
    tx.execute(
        r#"
        UPDATE inference_session_kv_residency
        SET owner_device_id = ?,
            residency_kind = CASE
                WHEN replica_device_id = ? THEN 'checkpoint_store'
                ELSE 'remote_reference'
            END,
            status = 'ready',
            sequence_first_position = 0,
    ...
    tx.execute(
        ...
        WHERE transfer_id IN (
            ...
            FROM inference_session_kv_transfers
            WHERE job_id = ?
              AND status IN ('completed', 'stale', 'failed')
        )
        "#,
        params![job_id],
    )
...
fn mark_session_kv_tracking_terminal(
    tx: &rusqlite::Transaction<'_>,
    job_id: &str,
    job_status: &str,
    error: Option<&str>,
    ...
    tx.execute(
        ...
        UPDATE inference_session_kv_residency
        SET status = CASE
                WHEN ? IN ('completed', 'failed') THEN 'stale'
                ELSE status
            END,
            eviction_eligible = CASE
                WHEN ? IN ('completed', 'failed') THEN 1
                ELSE eviction_eligible
            END,
            pinned_for_decode = CASE
                WHEN ? IN ('completed', 'failed') THEN 0
                ELSE pinned_for_decode
            END,
            updated_at = ?,
            last_error = COALESCE(last_error, ?)
        WHERE job_id = ?
        "#,
        params![job_status, job_status, job_status, now, error, job_id],
    )
    .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
    tx.execute(
        r#"
        ...
        WHERE job_id = ?
        "#,
        params![job_status, now, error, job_id],
    )
    .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
    tx.execute(
        r#"
        ...
        SET status = CASE
            ...
            updated_at = ?,
            last_error = CASE
                WHEN ? = 'failed' THEN COALESCE(last_error, ?)
                ELSE last_error
            END
        WHERE job_id = ?
        "#,
        params![job_status, now, job_status, error, job_id],
    )
    .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
    Ok(())
}
...
fn sync_serving_groups(
    ...
    for group in &plan.execution_groups {
        ...
        for member in &group.members {
            let backend_contract_json =
                serde_json::to_string(&member.backend_contract).map_err(|e| {
                    ApiError::Internal(format!(
    ...
    for group in &plan.support_groups {
        let support_role = serialize_support_group_role(group.role);
        for member in &group.members {
            let backend_contract_json =
                serde_json::to_string(&member.backend_contract).map_err(|e| {
                    ApiError::Internal(format!(
                        "Failed to serialize support-group backend contract: {}",
                        e
                    ))
                })?;
            let status = match group.role {
                SupportGroupRole::Kv => "kv_member",
                SupportGroupRole::Checkpoint => "checkpoint_member",
                SupportGroupRole::Recovery => "recovery_member",
                SupportGroupRole::Overflow => "overflow_member",
            };
            conn.execute(
                r#"
                INSERT INTO inference_serving_groups (
                ...
                params![
                    ...
                    serde_json::to_string(&group.compatibility_class).map_err(|e| {
                        ApiError::Internal(format!(
                            "Failed to serialize support-group compatibility class: {}",
                            e
                        ))
                    })?,
                    group.backend_contract_hash.clone(),
                    ...
                    serde_json::to_string(&group.protocol_class).map_err(|e| {
                        ApiError::Internal(format!(
                            "Failed to serialize support-group protocol class: {}",
                            e
                        ))
                    })?,
                    backend_contract_json,
...
fn serialize_support_group_role(role: SupportGroupRole) -> &'static str {
    match role {
...
fn compute_decode_lease_targets(
    ...
    let compatible_session_count =
        ...
        .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))? as u32;

    let target_session_count = compatible_session_count.max(1);
    Ok((target_session_count, target_session_count))
}
...
fn load_decode_lease_member_session_ids(
        ...
        .query_row(
            ...
            |row| {
                Ok((
                    ...
                    row.get::<_, String>(4)?,
                    row.get::<_, Option<i64>>(5)?.map(|value| value as u32),
                ))
            },
        )
        .optional()
    ...
    let pooled_group_key = batch_group_key.unwrap_or(group_id);
    let (target_session_count, _) =
        resolve_session_decode_lease_targets(conn, session_id, lease_target_session_count, None)?;
    let target_session_count =
        usize::try_from(target_session_count.unwrap_or(1).max(1)).unwrap_or(usize::MAX);
    let mut stmt = conn
        .prepare(
            r#"
            SELECT session_id
            ...
            ORDER BY
                CASE
                    ...
                    WHEN lease_owner_device_id = ? AND status IN ('leased', 'active') THEN 1
                    WHEN lease_owner_device_id = ? THEN 2
                    WHEN status IN ('leased', 'active') THEN 3
                    ELSE 4
                END ASC,
                COALESCE(ready_at, updated_at) ASC,
                session_id ASC
            "#,
        )
        .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
    let mut session_ids = stmt
        .query_map(
            params![
                network_id,
                pooled_group_key,
        ...
        .collect::<Result<Vec<_>, _>>()
        .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
    if session_ids.len() > target_session_count {
        session_ids.truncate(target_session_count);
    }
    Ok(session_ids)
}

fn load_decode_lease_session_members(
    conn: &rusqlite::Connection,
    device_id: &str,
    session_ids: &[String],
) -> ApiResult<Vec<DecodeLeaseSessionMember>> {
    let mut members = Vec::with_capacity(session_ids.len());
    for session_id in session_ids {
        let (job_id, segment_id) = conn
            .query_row(
                "SELECT job_id, segment_id FROM inference_decode_queue WHERE session_id = ?",
                params![session_id],
                |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?)),
            )
            .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        let session =
            load_serving_session_metadata(conn, session_id, device_id)?.ok_or_else(|| {
                ApiError::Internal(format!(
                    "decode lease cohort session {} was not visible in serving metadata",
                    session_id
                ))
            })?;
        members.push(DecodeLeaseSessionMember {
            job_id,
            session_id: session.session_id,
            segment_id,
            active_segment_id: session.active_segment_id,
            ...
            local_replica: session.local_replica,
        });
    }
    Ok(members)
}
...
fn decode_batch_group_key(plan: &InferenceExecutionPlan, segment_id: &str) -> ApiResult<String> {
    ...
    Ok(format!(
        "decode:{}:{}",
        group.model_id, group.execution_island_id
    ))
}

fn describe_decode_topology_change(
    previous: &InferenceExecutionPlan,
    refreshed: &InferenceExecutionPlan,
) -> Option<String> {
    let previous_decode = previous
        .execution_groups
        .iter()
...
fn sorted_member_ids(members: &[crate::api::types::ExecutionGroupMember]) -> Vec<String> {
    let mut ids = members
        .iter()
...
pub async fn report_inference_progress(
    ...
    if req.device_id.is_empty() || req.session_id.is_empty() || job_id.is_empty() {
        return Err(ApiError::BadRequest(
            "device_id, session_id, and job_id must be provided".to_string(),
        ));
    }
...
pub async fn report_inference_session_kv_transfer(
    State(state): State<AppState>,
    Path(job_id): Path<String>,
    Json(req): Json<ReportInferenceSessionKvTransferRequest>,
) -> ApiResult<Json<ReportInferenceSessionKvTransferResponse>> {
    if req.device_id.is_empty() || req.session_id.is_empty() || req.segment_id.is_empty() {
        return Err(ApiError::BadRequest(
            "device_id, session_id, and segment_id must be provided".to_string(),
        ));
...
pub async fn upload_inference_session_kv_transfer_payload(
    State(state): State<AppState>,
    Path(job_id): Path<String>,
    Json(req): Json<UploadInferenceSessionKvTransferPayloadRequest>,
) -> ApiResult<Json<serde_json::Value>> {
    if req.device_id.is_empty()
        || req.session_id.is_empty()
        || req.segment_id.is_empty()
        || req.transfer_id.is_empty()
    {
        return Err(ApiError::BadRequest(
            "device_id, session_id, segment_id, and transfer_id must be provided".to_string(),
        ));
    }
...
pub async fn download_inference_session_kv_transfer_payload(
    State(state): State<AppState>,
    Path((job_id, transfer_id)): Path<(String, String)>,
    Query(query): Query<DownloadKvTransferPayloadQuery>,
) -> ApiResult<Json<DownloadInferenceSessionKvTransferPayloadResponse>> {
    if job_id.is_empty() || transfer_id.is_empty() || query.device_id.is_empty() {
        return Err(ApiError::BadRequest(
            "job_id, transfer_id, and device_id must be provided".to_string(),
        ));
    }
    ...
    Ok(Json(DownloadInferenceSessionKvTransferPayloadResponse {
        success: true,
        transfer: payload.as_ref().map(|item| item.0.clone()),
        payload_hex: payload.map(|item| hex::encode(item.1)),
    }))
}
...
fn load_device_assignment_metadata_from_connection(
    ...
    let telemetry = load_device_runtime_telemetry_from_connection(
        conn,
        network_id,
        device_id,
        model_id,
    ...
    if !model_id.is_empty() {
        let multiplier = load_recent_device_throughput_multiplier_from_connection(
            conn, network_id, device_id, model_id,
        )?;
...
fn load_device_runtime_telemetry_from_connection(
    conn: &rusqlite::Connection,
    network_id: &str,
    device_id: &str,
    model_id: &str,
    ...
    let mut telemetry = DeviceRuntimeTelemetry {
        instability_score: scheduler_runtime_instability_score(
            status,
            last_seen,
    ...
    ) in rows
    {
        ...
        if let (Some(active), Some(deferred)) = (active_decode_sessions, deferred_decode_sessions) {
            deferred_ratio_total += (deferred / active.max(1.0)).clamp(0.0, 1.0);
...
fn scheduler_runtime_instability_score(
    status: &str,
    last_seen: Option<&str>,
    connectivity_state_json: Option<&str>,
    listen_addrs_json: Option<&str>,
    ...
    if let Some(json) = connectivity_state_json {
        if let Ok(state) =
            serde_json::from_str::<crate::connectivity::DeviceConnectivityState>(json)
        {
    ...
    if let Some(last_seen) = last_seen {
        if let Ok(parsed) =
            time::OffsetDateTime::parse(last_seen, &time::format_description::well_known::Rfc3339)
        {
            let age = (time::OffsetDateTime::now_utc() - parsed).whole_milliseconds();
            if age > 1_500 {
                score = score.saturating_add(15);
            }
            if age > 5_000 {
...
fn load_recent_device_throughput_multiplier_from_connection(
    conn: &rusqlite::Connection,
    network_id: &str,
    device_id: &str,
    model_id: &str,
) -> ApiResult<f64> {
    let rows = load_recent_device_service_rate_rows_from_connection(conn, network_id, model_id)?;
    Ok(throughput_multiplier_for_device_rows(&rows, device_id))
}

fn load_recent_device_service_rate_rows_from_connection(
    conn: &rusqlite::Connection,
    network_id: &str,
    model_id: &str,
) -> ApiResult<Vec<(String, f64)>> {
    let total_columns = model_assets::load_model_manifest(model_id)
    ...
    let rows = stmt
        .query_map(params![network_id, model_id], |row| {
            ...
            let shard_fraction = shard_columns / total_columns as f64;
...
fn throughput_multiplier_for_device_rows(rows: &[(String, f64)], device_id: &str) -> f64 {
...
fn claim_assignment(
    ...
    let result = (|| -> ApiResult<Option<PersistedAssignment>> {
        ...
        row.map(|assignment| enrich_claimed_assignment(&conn, assignment))
            .transpose()
    })();
...
fn report_assignment_progress(
    ...
    let scheduling_policy = if prefill_complete {
        ...
        let network_id: String = conn
            ...
            .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        Some(state.get_network_scheduling_policy(&network_id)?)
    } else {
        None
    };
    ...
    let assignment_updated = if prefill_complete {
        tx.execute(
            r#"
            UPDATE inference_job_assignments
        ...
        tx.execute(
            ...
            SET reported_completion_tokens = MAX(reported_completion_tokens, ?),
                execution_time_ms = MAX(execution_time_ms, ?)
            WHERE job_id = ? AND device_id = ? AND active_segment_id = ?
              AND status IN ('pending', 'leased', 'acknowledged', 'completed')
            "#,
            params![
                req.completion_tokens as i64,
                req.execution_time_ms as i64,
    ...
    if prefill_complete && assignment_updated == 0 {
        return Err(ApiError::NotFound(format!(
    ...
    if !prefill_complete {
        ...
        if !session_exists {
            return Err(ApiError::NotFound(format!(
                "Session {} not found for decode progress on job {}",
                req.session_id, job_id
            )));
        }
            ...
            || req.deferred_decode_sessions.is_some()
        {
            ...
            tx.execute(
                ...
                params![
                    ...
                    req.execution_time_ms as i64,
                    req.batch_size.map(i64::from),
                    req.target_session_count
                        .or_else(|| decode_batch_targets.as_ref().and_then(|targets| targets.0))
                        .map(i64::from),
                    req.target_batch_size
                        .or_else(|| decode_batch_targets.as_ref().and_then(|targets| targets.1))
                        .map(i64::from),
                    req.active_decode_sessions.map(i64::from),
                    req.batch_kv_tokens.map(i64::from),
                    req.deferred_decode_sessions.map(i64::from),
    ...
    if prefill_complete {
        ...
        if all_prefill_participants_completed {
            if let Some(refreshed_plan) = refresh_decode_plan_for_job(
                ...
                let topology_change =
                    describe_decode_topology_change(&execution_plan, &refreshed_plan);
                ...
                .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
                if let Some(detail) = topology_change.as_deref() {
                    let refreshed_decode_segment = refreshed_plan.segments.iter().find(|segment| {
                        matches!(segment.phase, crate::api::types::ExecutionPhase::Decode)
                    });
                    let refreshed_decode_group =
                        refreshed_plan.execution_groups.iter().find(|group| {
                            matches!(group.phase, crate::api::types::ExecutionPhase::Decode)
                        });
                    let batch_group_key = refreshed_decode_segment.and_then(|segment| {
                        decode_batch_group_key(&refreshed_plan, &segment.segment_id).ok()
                    });
                    record_scheduler_event(
                        &tx,
                        SchedulerEventRecord {
                            ...
                            job_id: Some(job_id),
                            session_id: refreshed_decode_segment
                                .map(|segment| segment.session_id.as_str()),
                            device_id: Some(&req.device_id),
                            segment_id: refreshed_decode_segment
                                .map(|segment| segment.segment_id.as_str()),
                            group_id: refreshed_decode_group.map(|group| group.group_id.as_str()),
                            batch_group_key: batch_group_key.as_deref(),
                            event_kind: "decode_topology_refreshed",
                            queue_status: None,
                            detail: Some(detail),
                            lease_target_session_count: None,
                            lease_target_batch_size: None,
            ...
            if let Some(next_segment_id) = next_segment_id(&execution_plan, &req.segment_id) {
                ...
                if decode_queue_status == "blocked_on_transfer" {
                    plan_session_kv_transfers_for_decode_segment(
                        &tx,
                        job_id,
                        &job_context.network_id,
                        &job_context.model_id,
...
fn store_session_checkpoint(
    ...
    .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
    materialize_session_kv_tracking(
        &tx,
        job_id,
        &req.session_id,
        &req.device_id,
        &checkpoint_id,
        req.kv_sequence_position,
...
fn store_session_kv_transfer_report(
    db: &crate::db::Database,
    job_id: &str,
    ...
    if !matches!(
        ...
        return Err(ApiError::BadRequest(
            "status must be one of queued, in_progress, completed, or failed".to_string(),
        ));
    }
    ...
    let transfer = tx
        .query_row(
            r#"
            SELECT transfer_id, source_device_id, transfer_kind
            FROM inference_session_kv_transfers
            WHERE job_id = ? AND session_id = ? AND segment_id = ? AND target_device_id = ?
            "#,
            params![job_id, &req.session_id, &req.segment_id, &req.device_id],
            |row| {
                Ok((
                    row.get::<_, String>(0)?,
                    row.get::<_, String>(1)?,
        ...
        .ok_or_else(|| {
            ApiError::NotFound(format!(
                "KV transfer not found for job {}, session {}, segment {}, target {}",
                job_id, req.session_id, req.segment_id, req.device_id
            ))
        })?;
    ...
    tx.execute(
        ...
        UPDATE inference_session_kv_transfers
        SET status = ?,
            remote_access_uri = COALESCE(?, remote_access_uri),
            kv_sequence_position = COALESCE(?, kv_sequence_position),
            bytes_total = COALESCE(?, bytes_total),
            bytes_transferred = COALESCE(?, bytes_transferred),
            started_at = CASE
        ...
        params![
            &req.status,
            req.remote_access_uri.as_deref(),
            req.kv_sequence_position.map(i64::from),
            req.bytes_total.map(|value| value as i64),
            req.bytes_transferred.map(|value| value as i64),
            &req.status,
            &now,
            &req.status,
            &now,
    ...
    if req.status == "completed" {
        tx.execute(
            ...
            SET owner_device_id = ?,
                residency_kind = CASE
                    WHEN ? = 'live_kv_handoff' THEN 'live_resident'
                    ELSE residency_kind
                END,
                status = 'ready',
                sequence_first_position = COALESCE(sequence_first_position, 0),
                sequence_next_position = COALESCE(?, sequence_next_position),
                ...
                updated_at = ?,
                last_error = NULL
            WHERE job_id = ? AND session_id = ? AND phase = 'decode' AND replica_device_id = ?
            "#,
            params![
                &transfer.1,
                &transfer.2,
        ...
        .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        tx.execute(
            "DELETE FROM inference_session_kv_transfer_payloads WHERE transfer_id = ?",
            params![&transfer.0],
        )
        .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        tx.execute(
        ...
        tx.execute(
            ...
            SET status = CASE
                ...
                updated_at = ?,
                last_error = NULL
            WHERE job_id = ? AND session_id = ? AND phase = 'decode' AND device_id = ?
            "#,
            params![&now, job_id, &req.session_id, &req.device_id],
        )
        .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        tx.execute(
            r#"
            UPDATE inference_sessions
            SET kv_sequence_position = MAX(COALESCE(kv_sequence_position, 0), COALESCE(?, 0)),
                updated_at = ?,
                last_error = NULL
            WHERE job_id = ? AND session_id = ?
            "#,
        ...
        let remaining_transfers: i64 = tx
            .query_row(
                r#"
                SELECT COUNT(*)
                FROM inference_session_kv_transfers
                WHERE job_id = ? AND session_id = ? AND segment_id = ?
                  AND status IN ('queued', 'in_progress')
                "#,
                params![job_id, &req.session_id, &req.segment_id],
                |row| row.get(0),
            )
            .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        if remaining_transfers == 0 {
            tx.execute(
                ...
                WHERE job_id = ? AND session_id = ? AND segment_id = ?
                "#,
                params![&now, &now, job_id, &req.session_id, &req.segment_id],
            )
            .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
            tx.execute(
                r#"
                ...
                WHERE job_id = ? AND session_id = ?
                "#,
                params![&now, job_id, &req.session_id],
            )
            .map_err(|e| ApiError::Database(Box::new(crate::db::DbError::Rusqlite(e))))?;
        }
...
fn store_session_kv_transfer_payload(
    db: &crate::db::Database,
    job_id: &str,
    req: &UploadInferenceSessionKvTransferPayloadRequest,
) -> ApiResult<()> {
    let payload_bytes = hex::decode(&req.payload_hex).map_err(|e| {
        ApiError::BadRequest(format!("payload_hex must be valid hexadecimal: {}", e))
    })?;
    if payload_bytes.is_empty() {
        return Err(ApiError::BadRequest(
            "payload_hex must not be empty".to_string(),
        ));
    }
    ...
    let (session_id, segment_id, source_device_id, target_device_id, status): (
        ...
        .query_row(
            r#"
            SELECT session_id, segment_id, source_device_id, target_device_id, status
            FROM inference_session_kv_transfers
            WHERE transfer_id = ? AND job_id = ?
            "#,
            params![&req.transfer_id, job_id],
            |row| {
                Ok((
                    row.get(0)?,
                    row.get(1)?,
        ...
        .ok_or_else(|| {
            ApiError::NotFound(format!(
                "KV transfer {} not found for job {}",
                req.transfer_id, job_id
            ))
        })?;

    if session_id != req.session_id || segment_id != req.segment_id {
        return Err(ApiError::BadRequest(
            "transfer payload identifiers did not match the queued transfer".to_string(),
        ));
    }
    if source_device_id != req.device_id {
        return Err(ApiError::BadRequest(
            "only the transfer source may upload a KV transfer payload".to_string(),
        ));
    }
    if matches!(status.as_str(), "completed" | "failed" | "stale") {
        return Err(ApiError::BadRequest(
            "cannot upload payload for a terminal KV transfer".to_string(),
        ));
    }
    ...
    let payload_size_bytes = payload_bytes.len() as u64;
    ...
    tx.execute(
        ...
        ON CONFLICT(transfer_id) DO UPDATE SET
            kv_sequence_position = COALESCE(excluded.kv_sequence_position, inference_session_kv_transfer_payloads.kv_sequence_position),
...
fn load_session_kv_transfer_payload(
    db: &crate::db::Database,
    job_id: &str,
    transfer_id: &str,
    device_id: &str,
) -> ApiResult<Option<(PendingKvTransferStatus, Vec<u8>)>> {
    let conn = db.get_conn()?;
    conn.query_row(
        r#"
        SELECT t.transfer_id, t.session_id, t.job_id, t.network_id, t.model_id, t.segment_id,
               t.group_id, t.batch_group_key, t.source_device_id, t.target_device_id,
               t.transfer_kind, t.status, t.kv_sequence_position, t.bytes_total,
               t.bytes_transferred, t.started_at, t.completed_at, t.updated_at, t.last_error,
               p.payload_bytes
        FROM inference_session_kv_transfers t
        JOIN inference_session_kv_transfer_payloads p ON p.transfer_id = t.transfer_id
        WHERE t.job_id = ? AND t.transfer_id = ?
          AND t.target_device_id = ?
        "#,
        params![job_id, transfer_id, device_id],
        |row| {
            Ok((
                PendingKvTransferStatus {
                    transfer_id: row.get(0)?,
                    session_id: row.get(1)?,
                    job_id: row.get(2)?,
                    network_id: row.get(3)?,
                    ...
                    kv_sequence_position: row.get::<_, Option<i64>>(12)?.map(|v| v as u32),
                    bytes_total: row.get::<_, Option<i64>>(13)?.map(|v| v as u64),
                    bytes_transferred: row.get::<_, Option<i64>>(14)?.map(|v| v as u64),
...
mod tests {
        ...
        for device_id in prefill_devices {
            ...
            let _ = report_inference_progress(
                ...
                Json(ReportInferenceAssignmentProgressRequest {
                    device_id,
                    session_id: claim.session.session_id.clone(),
    ...
    async fn test_realtime_progress_records_batch_telemetry_without_midflight_settlement() {
        ...
        for device_id in ["worker-1", "worker-2"] {
            ...
            let _ = report_inference_progress(
                ...
                Json(ReportInferenceAssignmentProgressRequest {
                    device_id: device_id.into(),
                    session_id: prefill_claim.session.session_id.clone(),
                    segment_id: prefill_segment_id.clone(),
                    phase: ExecutionPhase::Prefill,
                    event: ProgressEventKind::PrefillComplete,
                    completion_tokens: 1,
        ...
        let _ = report_inference_progress(
            ...
            Json(ReportInferenceAssignmentProgressRequest {
                device_id: "worker-1".into(),
                session_id: decode_claim.session.session_id.clone(),
    ...
    async fn test_uploaded_session_checkpoint_is_exposed_on_status_lease_and_download() {
        ...
        let residency = scheduler
            .kv_residency
            .iter()
            .find(|entry| entry.session_id == session_id)
            .expect("expected kv residency");
        let expected_remote_uri = format!(
            "/api/inference/jobs/{}/session-checkpoints/{}",
            submit.job_id, session_id
        );
        assert!(
            residency
                .residency_slices
                .iter()
                .any(|slice| slice.checkpoint_id.as_deref()
        ...
        assert!(residency.prompt_cache_entries.iter().any(|entry| {
            entry.status == "ready"
                && entry.remote_access_uri.as_deref() == Some(expected_remote_uri.as_str())
        }));
        assert!(residency
            .compute_hint
            .as_ref()
            .map(|hint| hint.preferred_device_ids.contains(&"worker-2".to_string()))
            .unwrap_or(false));
    ...
    #[tokio::test]
    async fn test_kv_transfer_payload_roundtrip_is_target_scoped_and_cleans_up_on_completion() {
        let network_id = "test-network-kv-transfer-payload-roundtrip";
        ...
        let target_pending = observe_pending_kv_transfers(
            State(state.clone()),
            Query(ObservePendingKvTransfersQuery {
                device_id: "worker-3".into(),
                network_id: network_id.into(),
            }),
        )
        ...
        .unwrap()
        .0;
        let transfer = target_pending
            .transfers
            .into_iter()
            .find(|item| item.target_device_id == "worker-3")
            .expect("expected pending worker-3 transfer");
        assert!(!transfer.payload_available);

        let _ = upload_inference_session_kv_transfer_payload(
            State(state.clone()),
            Path(submit.job_id.clone()),
            Json(UploadInferenceSessionKvTransferPayloadRequest {
                device_id: transfer.source_device_id.clone(),
                session_id: session_id.clone(),
                segment_id: transfer.segment_id.clone(),
                transfer_id: transfer.transfer_id.clone(),
                payload_hex: "c0ffee".into(),
                kv_sequence_position: Some(1),
            }),
        )
        .await
        .unwrap();

        let target_pending_after_upload = observe_pending_kv_transfers(
            State(state.clone()),
            Query(ObservePendingKvTransfersQuery {
                device_id: "worker-3".into(),
                network_id: network_id.into(),
        ...
        .unwrap()
        .0;
        let target_transfer = target_pending_after_upload
            .transfers
            .into_iter()
            .find(|item| item.transfer_id == transfer.transfer_id)
            .expect("expected target transfer");
        assert!(target_transfer.payload_available);

        let unauthorized_download = download_inference_session_kv_transfer_payload(
            State(state.clone()),
            Path((submit.job_id.clone(), transfer.transfer_id.clone())),
            Query(DownloadKvTransferPayloadQuery {
                device_id: "worker-1".into(),
            }),
        )
        .await
        .unwrap()
        .0;
        assert!(unauthorized_download.transfer.is_none());
        assert!(unauthorized_download.payload_hex.is_none());

        let payload_download = download_inference_session_kv_transfer_payload(
            State(state.clone()),
            Path((submit.job_id.clone(), transfer.transfer_id.clone())),
            Query(DownloadKvTransferPayloadQuery {
                device_id: "worker-3".into(),
        ...
        .unwrap()
        .0;
        assert_eq!(payload_download.payload_hex.as_deref(), Some("c0ffee"));

        let _ = report_inference_session_kv_transfer(
            State(state.clone()),
            Path(submit.job_id.clone()),
            Json(ReportInferenceSessionKvTransferRequest {
                device_id: "worker-3".into(),
                session_id: session_id.clone(),
                segment_id: transfer.segment_id.clone(),
        ...
        let post_status =
            get_inference_job_status(State(state.clone()), Path(submit.job_id.clone()))
                ...
                .unwrap()
                .0;
        let post_session = post_status.session.expect("expected session");
        assert!(post_session
            .replicas
            .iter()
            .any(|replica| replica.device_id == "worker-3" && replica.status == "decode_ready"));

        let conn = state.db.get_conn().unwrap();
        let payload_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM inference_session_kv_transfer_payloads WHERE transfer_id = ?",
    ...
    async fn test_decode_batch_targets_pool_across_shared_decode_participants() {
        ...
        let mut claimed_member_session_ids = decode_claim
            .session
            .lease_session_members
            .iter()
            .map(|member| member.session_id.clone())
            .collect::<Vec<_>>();
        claimed_member_session_ids.sort();
        assert_eq!(claimed_member_session_ids, expected_session_ids);
        assert_eq!(decode_claim.session.lease_session_members.len(), 2);
        ...
        assert_eq!(
            queue_statuses,
            vec!["leased".to_string(), "leased".to_string()]
        );
        ...
        let follower_member = decode_claim
            .session
            .lease_session_members
            .iter()
            .find(|member| member.job_id != decode_claim.job_id)
            .cloned()
            .expect("expected pooled follower member");
        let follower_segment_id = follower_member
            .active_segment_id
            .clone()
            .expect("expected pooled follower segment id");
        let _ = report_inference_progress(
            State(state.clone()),
            Path(follower_member.job_id.clone()),
            Json(ReportInferenceAssignmentProgressRequest {
                device_id: "worker-1".into(),
                session_id: follower_member.session_id.clone(),
                segment_id: follower_segment_id.clone(),
                phase: ExecutionPhase::Decode,
                event: ProgressEventKind::DecodeProgress,
                completion_tokens: 2,
                execution_time_ms: 75,
                time_to_first_token_ms: None,
                kv_cache_seq_len: Some(3),
                batch_size: Some(2),
                active_decode_sessions: Some(2),
                batch_kv_tokens: Some(96),
                deferred_decode_sessions: Some(0),
        ...
        let follower_status =
            get_inference_job_status(State(state.clone()), Path(follower_member.job_id.clone()))
                .await
                .unwrap()
                .0;
        assert_eq!(
        ...
        assert_eq!(
            follower_status
                ...
                .as_ref()
                .and_then(|session| session.recent_decode_batches.first())
                .map(|event| event.segment_id.as_str()),
            Some(follower_segment_id.as_str())
        );
    ...
    async fn test_decode_lease_renew_and_release_apply_to_owned_pooled_cohort() {
        ...
        for prompt in ["first", "second"] {
            ...
            let decode_segment = decode_plan
                ...
                .clone();
            let computed_batch_group_key =
                decode_batch_group_key(&decode_plan, &decode_segment.segment_id)
                    .expect("batch key");
            let conn = state.db.get_conn().unwrap();
            conn.execute(
                "UPDATE inference_jobs SET execution_plan_json = ? WHERE job_id = ?",
    ...
    #[tokio::test]
    async fn test_acknowledge_decode_promotes_owned_pooled_cohort_to_active() {
        let network_id = "test-network-pooled-decode-acknowledge";
        let state = joined_state(&["worker-1", "worker-2"], network_id).await;
        let mut job_ids = Vec::new();
        let mut pooled_group_key = None::<String>;
        let mut pooled_group_id = None::<String>;
    ...
    #[tokio::test]
    async fn test_decode_batch_targets_include_blocked_prefill_siblings_in_pooled_target() {
        let network_id = "test-network-pooled-blocked-prefill-targets";
        let state = joined_state(&["worker-1", "worker-2"], network_id).await;
        let mut job_ids = Vec::new();
        let mut pooled_group_key = None::<String>;
        let mut pooled_group_id = None::<String>;
            ...
            conn.execute(
                ...
                SET status = ?,
                    active_segment_id = CASE
                        ...
                        ELSE active_segment_id
                    END,
                    kv_sequence_position = CASE
                        WHEN ? = 'ready' THEN 1
                        ELSE kv_sequence_position
                    END,
                    updated_at = datetime('now')
                WHERE job_id = ?
                "#,
    ...
    #[tokio::test]
    async fn test_decode_batch_targets_ignore_serialized_observed_peak_history() {
        let network_id = "test-network-pooled-ignore-serialized-history";
        let state = joined_state(&["worker-1", "worker-2"], network_id).await;
        let mut pooled_group_key = None::<String>;
        let mut pooled_group_id = None::<String>;
        ...
        for prompt in ["one", "two"] {
            ...
            let (session_id, batch_group_key): (String, Option<String>) = conn
                .query_row(
                    "SELECT session_id, batch_group_key FROM inference_decode_queue WHERE job_id = ?",
                    params![submit.job_id],
                    |row| Ok((row.get(0)?, row.get(1)?)),
                )
                .unwrap();
            ...
            conn.execute(
                ...
                WHERE session_id = ?
                "#,
                params![&decode_segment.segment_id, &session_id],
            )
            .unwrap();
            conn.execute(
                r#"
    ...
    #[tokio::test]
    async fn test_any_claim_prefers_prefill_for_blocked_decode_cohort_before_ready_decode() {
        let network_id = "test-network-prefill-before-ready-decode-cohort";
        let state = joined_state(&["worker-1", "worker-2"], network_id).await;

        let ready_submit = submit_inference(
            State(state.clone()),
            Json(SubmitInferenceRequest {
                request_id: Uuid::new_v4().to_string(),
                device_id: "worker-1".into(),
        ...
        .unwrap()
        .0;
        let ready_plan = ready_submit
            .execution_plan
            .clone()
            .expect("expected ready execution plan");
        let ready_status =
            drive_job_to_decode_ready(&state, &ready_submit.job_id, network_id, &ready_plan).await;
        ...
        let blocked_submit_a = submit_inference(
            State(state.clone()),
            Json(SubmitInferenceRequest {
                request_id: Uuid::new_v4().to_string(),
                device_id: "worker-1".into(),
        ...
        .unwrap()
        .0;
        let blocked_submit_b = submit_inference(
            State(state.clone()),
            Json(SubmitInferenceRequest {
                request_id: Uuid::new_v4().to_string(),
                device_id: "worker-1".into(),
        ...
        let conn = state.db.get_conn().unwrap();
        let batch_group_keys = [
            ready_submit.job_id.as_str(),
            blocked_submit_a.job_id.as_str(),
            blocked_submit_b.job_id.as_str(),
        ]
        .into_iter()
        .map(|job_id| {
            conn.query_row(
                "SELECT batch_group_key FROM inference_decode_queue WHERE job_id = ?",
                params![job_id],
                |row| row.get::<_, Option<String>>(0),
            )
            .unwrap()
            .expect("expected pooled batch group key")
        })
        .collect::<Vec<_>>();
        assert_eq!(batch_group_keys[0], batch_group_keys[1]);
        assert_eq!(batch_group_keys[1], batch_group_keys[2]);
        drop(conn);

        for blocked_job_id in [&blocked_submit_a.job_id, &blocked_submit_b.job_id] {
            let blocked_status =
                get_inference_job_status(State(state.clone()), Path(blocked_job_id.clone()))
                    .await
                    .unwrap()
                    .0;
            assert_eq!(blocked_status.status, "dispatched");
            let conn = state.db.get_conn().unwrap();
            let queue_status: String = conn
                .query_row(
                    "SELECT status FROM inference_decode_queue WHERE job_id = ?",
                    params![blocked_job_id],
                    |row| row.get(0),
                )
                .unwrap();
            drop(conn);
            assert_eq!(queue_status, "blocked_on_prefill");
        }
        ...
        assert_eq!(claim.active_segment.phase, ExecutionPhase::Prefill);
        assert_eq!(claim.job_id, blocked_submit_a.job_id);
        assert_ne!(claim.job_id, ready_submit.job_id);
    }

    #[tokio::test]
    async fn test_decode_lease_member_ids_include_ready_siblings_during_lease_attach_race() {
        let network_id = "test-network-decode-lease-race";
        let state = joined_state(&["worker-1", "worker-2"], network_id).await;
        ...
        for prompt in ["first", "second"] {
            ...
            pooled_group_key = Some(batch_group_key.unwrap_or(group_id.clone()));
            pooled_group_id = Some(decode_segment.execution_group_id.clone());
        }
        ...
        let owner_session_id = session_ids[0].clone();
        let follower_session_id = session_ids[1].clone();
        conn.execute(
            r#"
            UPDATE inference_decode_queue
            SET status = 'leased',
                lease_owner_device_id = 'worker-1',
        ...
        conn.execute(
            ...
            WHERE session_id = ?
            "#,
            params![&follower_session_id],
        )
        .unwrap();
        drop(conn);

        let conn = state.db.get_conn().unwrap();
        let mut lease_session_ids =
            load_decode_lease_member_session_ids(&conn, &owner_session_id).unwrap();
        lease_session_ids.sort();
        let mut expected_session_ids = session_ids.clone();
        expected_session_ids.sort();
        assert_eq!(lease_session_ids, expected_session_ids);
    }
