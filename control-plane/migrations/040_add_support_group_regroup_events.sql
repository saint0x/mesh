...
DROP TRIGGER IF EXISTS trg_decode_queue_regroup_update;

CREATE TABLE inference_regroup_events_v2 (
...
INSERT INTO inference_regroup_events_v2 (
    event_id, session_id, job_id, network_id, model_id, phase, support_role, group_id, device_id,
    event_kind, reason, previous_status, new_status, segment_id, observed_at
)
SELECT
    event_id, session_id, job_id, network_id, model_id, phase, NULL, group_id, device_id,
    event_kind, reason, previous_status, new_status, segment_id, observed_at
FROM inference_regroup_events;

DROP TABLE inference_regroup_events;
ALTER TABLE inference_regroup_events_v2 RENAME TO inference_regroup_events;
...
BEGIN
    INSERT INTO inference_regroup_events (
        session_id, job_id, network_id, model_id, phase, support_role, group_id, device_id,
        event_kind, reason, previous_status, new_status, segment_id, observed_at
    ) VALUES (
        NEW.session_id, NEW.job_id, NEW.network_id, NEW.model_id, NEW.phase, NEW.support_role,
        NEW.group_id, NEW.device_id,
        'member_added',
        CASE
...
BEGIN
    INSERT INTO inference_regroup_events (
        ...
        OLD.session_id, OLD.job_id, OLD.network_id, OLD.model_id, OLD.phase, OLD.support_role,
        OLD.group_id, OLD.device_id,
        'member_removed',
        'group_row_removed',
        OLD.status,
        NULL,
