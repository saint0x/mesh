...
- [x] Persist authoritative execution plans at submit time
- [x] Represent explicit prefill and decode segments
- [x] Re-plan decode placement at prefill handoff against live pool state
- [x] Move claim-time assignment selection behind a dedicated scheduler service
...
- [x] Introduce authoritative serving-session and per-replica control-plane state
- [x] Support checkpoint-mediated KV handoff through the control plane
...
- [x] Introduce backend/runtime boundary in the agent
- [x] Preserve explicit session identity in runtime state
- [x] Preserve checkpoint-backed recovery for lost in-memory state
...
- [x] Add targeted Rust tests for explicit segment/session behavior
- [x] Add targeted Rust tests for decode-group shrink and final-segment completion
- [x] Add targeted Rust tests for checkpoint upload/download and handoff validation
- [x] Add deterministic Fozzy doctor/test/trace/replay/ci coverage for current dispatch path
...
- [x] Land authoritative planning, explicit segments, session authority, and checkpoint handoff
